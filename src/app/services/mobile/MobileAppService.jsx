    ...
    >
      ...
      <section className="py-20 bg-white">
        <Container>
          ...
          {(() => {
            const phases = [
              { title: "Project Kickoff", duration: "Week 1", description: "Requirements gathering and project planning", icon: "🚀" },
              { title: "UI/UX Design", duration: "Weeks 2-3", description: "Creating wireframes and visual designs", icon: "🎨" },
              { title: "Core Development", duration: "Weeks 4-9", description: "Building the main features and functionality", icon: "💻" },
              { title: "Testing & QA", duration: "Weeks 10-11", description: "Thorough testing and bug fixing", icon: "🔍" },
              { title: "Launch Preparation", duration: "Week 12", description: "Store submission and deployment", icon: "🎯" },
            ];
            return (
              <>
                {/* Mobile: linear stacked */}
                <div className="flex flex-col gap-5 md:hidden max-w-2xl mx-auto">
                  {phases.map((phase, i) => (
                    <motion.div key={phase.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="flex gap-4 items-start">
                ...
                <div className="hidden md:block max-w-5xl mx-auto">
                  <div className="relative">
                    ...
                    {phases.map((phase, index) => (
                      <motion.div key={phase.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className={`relative mb-12 ${index % 2 === 0 ? 'ml-[50%] pl-8' : 'mr-[50%] pr-8 text-right'}`}>
